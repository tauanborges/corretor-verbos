...
{% if error %}<div class="err"><b>{{error}}</b></div>{% endif %}
</div>
...
def get_daily_limit_mode():
    ...
    return MODE_UNLIMITED if value == MODE_UNLIMITED else MODE_LIMITED
...
textarea{width:100%;min-height:120px;padding:10px;font-size:16px;box-sizing:border-box}
button,.btnlink{padding:10px 14px;font-size:16px;cursor:pointer;border-radius:10px;border:1px solid #6b6b6b;background:#6b6b6b;color:#fff}
button:hover,.btnlink:hover{background:#5a5a5a}
.box{border:1px solid #e6e6e6;border-radius:14px;padding:16px;margin-top:16px;background:#fff;box-shadow:0 1px 10px rgba(0,0,0,.04)}
.muted{color:#666}.changes li{margin:6px 0}a{text-decoration:none}
.pill{display:inline-block;padding:6px 12px;border-radius:999px;background:#f4f4f4;color:#444;font-size:12px}
.header{margin:14px 0 18px}.logos{display:flex;gap:14px;align-items:center;flex-wrap:wrap;margin-bottom:10px}
.logos img{max-height:60px;max-width:220px;width:auto;height:auto;object-fit:contain}
.credit{margin:0;color:#444;background:#f7f7f7;border:1px solid #e6e6e6;padding:14px;border-radius:12px;line-height:1.5;font-size:16px}
h2{margin-top:26px;border-left:6px solid #6b6b6b;padding-left:12px}
.change-author{color:#777;font-size:13px;margin-left:5px}
</style>
</head>
<body>
<h1>{{title}}</h1>
...
<p class="muted">Digite uma frase e a ferramenta tentará corrigir com base nas regras cadastradas pela turma.</p>

<p><a class="btnlink" href="{{url_for('admin')}}">🔒 Ir para o painel</a></p>

<form method="post" action="{{url_for('home')}}">
<label><b>Frase do aluno</b></label><br>
<textarea name="text" placeholder="Digite aqui a frase que deseja analisar...">{{text or ""}}</textarea>
<br><br><button type="submit">✅ Corrigir</button>
</form>
...
{% if c.contributor %}
<span class="change-author">por @{{c.contributor}}</span>
{% endif %}
</li>
{% endfor %}
</ul>
...
input,textarea{width:100%;padding:10px;font-size:15px;box-sizing:border-box}
button,.filterbtn{padding:10px 14px;font-size:15px;cursor:pointer;border-radius:10px;border:1px solid #6b6b6b;background:#6b6b6b;color:#fff}
button:hover,.filterbtn:hover{background:#5a5a5a}
table{width:100%;border-collapse:collapse;margin-top:18px}
th,td{border-bottom:1px solid #eee;padding:10px;vertical-align:top;text-align:left}
.row{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.muted{color:#666}.danger{background:#fff6f6;border:1px solid #ffd0d0;padding:12px;border-radius:12px}
.info{background:#eef6ff;border:1px solid #b9dcff;padding:12px;border-radius:12px}
a{text-decoration:none}code{background:#f4f4f4;padding:2px 6px;border-radius:6px}
details{margin-top:16px}summary{cursor:pointer;font-weight:bold}
.btn-row{display:flex;gap:10px;flex-wrap:wrap;align-items:center}
.pill{display:inline-block;padding:6px 12px;border-radius:999px;background:#f4f4f4;color:#444;font-size:12px}
.box{border:1px solid #e6e6e6;border-radius:14px;padding:16px;margin-top:16px;background:#fff;box-shadow:0 1px 10px rgba(0,0,0,.04)}
h2{margin-top:30px;border-left:6px solid #6b6b6b;padding-left:12px}
.boards{display:grid;grid-template-columns:1fr 1fr;gap:14px}
@media(max-width:900px){.boards{grid-template-columns:1fr}.row{grid-template-columns:1fr}}
.leaderboard ol{margin:10px 0 0 22px;padding:0}.leaderboard li{margin:8px 0}
.medal{display:inline-block;min-width:26px;text-align:center}
.filters{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}
.msg{margin-top:12px}.warn{background:#fff7e6;border:1px solid #ffd9a8;padding:10px;border-radius:12px}
.ok{background:#ecfff1;border:1px solid #b6f2c4;padding:10px;border-radius:12px}
.dupwarn{display:none;margin-top:10px}.change-author{color:#777;font-size:13px;margin-left:5px}
</style>
</head>
<body>
...
<p><a class="filterbtn" href="{{url_for('home')}}">← Voltar para a ferramenta</a></p>

<div class="btn-row">
<form method="post" action="{{url_for('logout')}}"><button type="submit">Sair</button></form>
{% if role == "reviewer" %}
<a class="pill" href="{{url_for('admin_review')}}">📥 Fila de revisão: {{pending_count}} pendente(s)</a>
{% else %}
...
<h2>🏅 Top da Semana</h2>
<p class="muted">Somente regras aprovadas que entram no ranking (últimos 7 dias).</p>
{% if top_week %}
<ol>
{% for l in top_week %}
<li>
...
{% else %}<p class="muted">Ainda não há pontuações esta semana.</p>{% endif %}
</div>

<div class="box leaderboard">
<h2>🏆 Hall da Fama</h2>
<p class="muted">Ranking geral — quem mais ajudou a construir a base de regras.</p>
{% if hall %}
<ol>
{% for l in hall %}
<li>
...
{% else %}<p class="muted">Ainda não há contribuições no hall.</p>{% endif %}
</div>
</div>
...
<span class="muted">(por <b>@{{n.contributor or "—"}}</b>)</span>
</li>
{% endfor %}
</ul>
{% else %}<p class="muted">Nenhuma regra aprovada ainda.</p>{% endif %}
</div>

<div class="danger">
<b>Fluxo:</b> novas regras entram como <b>Pendentes</b> e serão revisadas pelo professor.
<br><span class="muted">Limite: alunos podem enviar até <b>5 regras por dia</b>, quando o limite estiver ativado.</span>
</div>
...
<div class="row">
<div><label><b>Forma errada</b></label><input id="wrong" name="wrong" placeholder="Ex.: nós vai" required value="{{prefill_wrong or ''}}"></div>
<div><label><b>Forma correta</b></label><input id="right" name="right" placeholder="Ex.: nós vamos" required value="{{prefill_right or ''}}"></div>
</div>
<br>
...
<textarea name="notes" placeholder="Ex.: comentário rápido">{{prefill_notes or ''}}</textarea>

<div id="dupwarn" class="dupwarn warn"><b>⚠️ Atenção:</b> essa regra já existe na base. O envio será bloqueado.</div>
<br><button id="submitBtn" type="submit">Enviar contribuição</button>
</form>
...
<div class="info">
<p class="muted">O PostgreSQL é o banco permanente da aplicação. O backup JSON continua disponível como camada extra de segurança.</p>
<div class="btn-row">
<form method="get" action="{{url_for('admin_export_download')}}"><button type="submit">Exportar regras (.json)</button></form>
<form method="post" action="{{url_for('admin_clear')}}" onsubmit="return confirm('Tem certeza que deseja APAGAR TODAS as regras?');"><button type="submit">Apagar todas as regras</button></form>
</div>
...
<br><br>
<label><input type="checkbox" name="replace_all" value="1"> Substituir tudo antes de importar</label>
<br><br><button type="submit">Importar</button>
</form>
{% if import_msg %}<p><b>{{import_msg}}</b></p>{% endif %}
</div>
</details>
{% endif %}
...
<table>
<thead><tr><th>Errado</th><th>Correto</th><th>Username</th><th>Status</th><th>Observação</th><th>Criada em</th><th>Revisada em</th><th>Ação</th></tr></thead>
<tbody>
{% for r in rules %}
<tr>
...
<td>
{% if role=="reviewer" %}
<form method="post" action="{{url_for('admin_delete',rule_id=r.id)}}" onsubmit="return confirm('Excluir esta regra?');"><button type="submit">Excluir</button></form>
{% else %}<span class="muted">—</span>{% endif %}
</td>
</tr>
{% endfor %}
{% if not rules %}<tr><td colspan="8" class="muted">Nenhuma regra para este filtro.</td></tr>{% endif %}
</tbody>
</table>
...
async function checkDup(){
 const w=(wrongEl.value||"").trim(), r=(rightEl.value||"").trim();
 if(!w||!r){warnEl.style.display="none";btnEl.disabled=false;return;}
 try{
  const qs=new URLSearchParams({wrong:w,right:r});
  const resp=await fetch("/api/check-duplicate?"+qs.toString());
  const data=await resp.json();
  warnEl.style.display=data.exists===true?"block":"none";
  btnEl.disabled=data.exists===true;
 }catch(e){warnEl.style.display="none";btnEl.disabled=false;}
}
...
button,.btnlink{padding:10px 14px;font-size:14px;cursor:pointer;border-radius:10px;border:1px solid #6b6b6b;background:#6b6b6b;color:#fff}
button:hover,.btnlink:hover{background:#5a5a5a}
table{width:100%;border-collapse:collapse;margin-top:18px}
th,td{border-bottom:1px solid #eee;padding:10px;vertical-align:top;text-align:left}
.muted{color:#666}code{background:#f4f4f4;padding:2px 6px;border-radius:6px}
.btns{display:flex;gap:8px;flex-wrap:wrap}.top{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
a{text-decoration:none}.pill{display:inline-block;padding:6px 12px;border-radius:999px;background:#f4f4f4;color:#444;font-size:12px}
.box{border:1px solid #e6e6e6;border-radius:14px;padding:16px;margin-top:16px}
</style>
</head>
<body>
<div class="top"><h1 style="margin:0">Fila de revisão</h1><span class="pill">{{pending_count}} pendente(s)</span></div>
<p><a class="btnlink" href="{{url_for('admin')}}">← Voltar para o painel</a></p>

{% if msg %}<p><b>{{msg}}</b></p>{% endif %}

<div class="box">
<b>Limite de contribuições:</b>
{% if daily_limit_mode == "limited" %}
5 regras por aluno/dia.
<form method="post" action="{{url_for('admin_toggle_daily_limit')}}" style="display:inline;margin-left:10px">
<input type="hidden" name="mode" value="unlimited"><button type="submit">Ativar ilimitado</button>
</form>
{% else %}
Ilimitado.
<form method="post" action="{{url_for('admin_toggle_daily_limit')}}" style="display:inline;margin-left:10px">
<input type="hidden" name="mode" value="limited"><button type="submit">Reativar limite de 5</button>
</form>
{% endif %}
</div>

<table>
<thead><tr><th>Errado</th><th>Correto</th><th>Username</th><th>Observação</th><th>Criada em</th><th>Decisão</th></tr></thead>
<tbody>
{% for r in pending %}
<tr>
<td><code>{{r.wrong}}</code></td><td><code>{{r.right}}</code></td>
<td class="muted">{{r.contributor or ""}}</td><td class="muted">{{r.notes or ""}}</td><td class="muted">{{r.created_at}}</td>
<td><div class="btns">
<form method="post" action="{{url_for('admin_review_decide',rule_id=r.id)}}"><input type="hidden" name="decision" value="rank"><button type="submit">Aprovar (pontua)</button></form>
<form method="post" action="{{url_for('admin_review_decide',rule_id=r.id)}}"><input type="hidden" name="decision" value="no_rank"><button type="submit">Aprovar (não pontua)</button></form>
<form method="post" action="{{url_for('admin_review_decide',rule_id=r.id)}}" onsubmit="return confirm('Marcar como NÃO aprovada?');"><input type="hidden" name="decision" value="not_approved"><button type="submit">Não aprovar</button></form>
</div></td>
</tr>
{% endfor %}
{% if not pending %}<tr><td colspan="6" class="muted">Nenhuma regra pendente no momento.</td></tr>{% endif %}
</tbody>
</table>
</body>
</html>
...
def admin_add():
    ...
    if not wrong or not right or not contributor:
        return redirect(url_for(
            "admin",
            ui_msg="Preencha errado, correto e username.",
            ui_kind="warn",
    ...
    if is_duplicate_rule(wrong, right):
        return redirect(url_for(
            "admin",
            ui_msg="Essa regra já existe na base. Envie uma diferente 🙂",
            ui_kind="warn",
            w=wrong, r=right, c=contributor, n=notes,
        ))

    if session.get("role") == ROLE_ADMIN and is_daily_limit_enabled():
    ...
    return redirect(url_for(
        "admin",
        ui_msg="Contribuição enviada! Agora ela vai para a revisão do professor ✅",
        ui_kind="ok",
    ))
...
def admin_toggle_daily_limit():
    mode = (request.form.get("mode", MODE_LIMITED) or MODE_LIMITED).strip().lower()

    if mode == MODE_UNLIMITED:
        set_daily_limit_mode(MODE_UNLIMITED)
        msg = "Envios ilimitados ativados."
    else:
        set_daily_limit_mode(MODE_LIMITED)
        msg = "Limite diário de 5 contribuições por aluno reativado."

    return redirect(url_for("admin_review", msg=msg))
...
def admin_export_download():
    ...
    resp.headers["Content-Disposition"] = 'attachment; filename="regras-backup.json"'
    return resp
...
@reviewer_required
def admin_import():
    json_payload = (request.form.get("json_payload", "") or "").strip()
    ...
    try:
        ...
        if not isinstance(rules, list):
            return redirect(url_for(
                "admin",
                msg="Erro: o JSON não tem uma lista válida em 'rules'.",
            ))
            ...
            for item in rules:
                ...
                if wrong and right:
                    cur.execute("""
                        ...
                        VALUES (%s, %s, %s, %s, %s, %s, %s)
                    """, (
                        wrong, right, notes, created_at,
                        contributor, status, reviewed_at,
                    ))
                    count += 1
        ...
        return redirect(url_for(
            "admin",
            msg=f"Importação concluída: {count} regra(s) adicionada(s).",
        ))

    except Exception as e:
        return redirect(url_for(
            "admin",
            msg=f"Erro ao importar JSON: {str(e)}",
        ))
...
try:
    ...
    # Não derruba o processo com uma mensagem pouco clara.
    # O Render exibirá o erro no log e o usuário verá uma
    # mensagem explicando que DATABASE_URL precisa ser configurada.
    print(f"[CONJUGA CIEBTEC] Banco ainda não inicializado: {startup_error}")
